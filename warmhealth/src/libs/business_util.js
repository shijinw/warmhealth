  ...
  for (let i = 0; i < menuList.length; i++) {
    ...
    if (item['pId'] == parentId) {
      if (item['menu_type'] == '1') {
        ...
        menuInfo['meta'] = {'title': item['title'], 'href': item['hrefUrl'], 'icon': item['icon']}
      }
      ...
      // }
        ...
        menuInfo['meta'] = {
          'title': item['title'],
          'icon': item['icon'],
          'hideInMenu': (parseInt(item['hideInMenu']) == 1),
          'hideInBread': (parseInt(item['hideInBread']) == 1),
          'notCache': (parseInt(item['notCache']) == 1),
          'beforeCloseName': item['beforeCloseName']
        }

        let children = menuRefactoring(menuList, item['id'])
        //console.log("菜单："+item['name']+",子级长度:"+children.length)
        if (children.length > 0) {
          menuInfo['component'] = "Main"
          menuInfo['children'] = children
          if (parentId=="-1") menuInfo["path"] = "/" + menuInfo["path"]
        } else {
          menuInfo['component'] = item["componentsUrl"]
        }
      }
...
// 从构菜单为 tree 结构
export const menuRefactoringTree = (menuList, parentId) => {
  let newMenu = [];
  for (let i = 0; i < menuList.length; i++) {
    let menuInfo = {};
    let item = menuList[i]
    if (item['pId'] == parentId) {
      if (item['type'] == '1') {
        ...
        let children = menuRefactoringTree(menuList, item['id'])
        if (children.length > 0) {
          menuInfo['component'] = () => import(item["componentsUrl"])
          menuInfo['children'] = children
